...
VALID_LABELS   = {"hand_biting", "head_hit", "hitting_others",
                  "scratching", "self_directed_hit",
...
def parse_chunk_response(text, chunk_duration):
    ...
    if not label:
        # Fallback: model dropped the "Label:" prefix and just output the answer
        # (e.g. bare "armflapping\n" or "Evidence: ... <gt_label> ..."). Prefer
        # any non-none valid behaviour token; fall back to "none".
        toks = re.findall(r"\b([a-z_]+)\b", text.lower())
        for tok in toks:
            if tok in VALID_LABELS and tok != "none":
                label = tok
                break
        if not label and "none" in toks:
            label = "none"
    if not evidence:
        m = re.search(
...
def make_prompt(chunk_dur, gt_label="", subject_hint=""):
    ...
    if gt_label and gt_label in defs:
        ...
        label_line   = f"none or {gt_label}"
    else:
        defn_block   = "\n".join(f"- {k}: {v}" for k, v in defs.items())
        label_line   = "none, " + ", ".join(defs.keys())
...
def classify_chunk(chunk):
    ...
    question = DEFAULT_IMAGE_TOKEN + "\n" + time_instruction + "\n" + make_prompt(chunk_dur, GT_LABEL, SUBJECT_HINT)
