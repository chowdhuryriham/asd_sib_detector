...
#     "yellow, golden-yellow, or brownish-yellow depending on the lighting. When filmed "
#     "from the front, the hoodie zip may be open, revealing a T-shirt underneath — this "
#     "is still the same child. Track only this child's behaviour throughout the clip."
# )
...
# 6.  Behaviour prompt
# ─────────────────────────────────────────────────────────
# def make_prompt(chunk_dur, GT_LABEL):
#     return (
...
def make_prompt(chunk_dur, gt_label="", subject_hint=""):
    ...
    if PROMPT_MODE == "rgb":
        ...
        return (
            ...
            f"{defn_block}\n\n"
            "NOISE NOTE: pose trackers may produce 1-frame keypoint jumps; treat a single isolated "
            "event as evidence ONLY if neighbouring frames show a continuous trajectory.\n\n"
            "Only label the behavior if you are confident the motion is clearly repetitive and stereotyped. If uncertain, output none.\n\n"
            "Reply in this exact format:\n"
...
def classify_chunk(chunk):
    ...
    # Adaptive frame count: clamp to native frame count for low-fps source videos.
    try:
        ...
        native_frames = int(_probe.stdout.strip() or 0)
    except Exception:
        native_frames = 0
    adaptive_frames = NUM_FRAMES
    if native_frames > 0:
        # 2-frame buffer + low floor (2) so ultra-short skeleton clips don't fail.
        adaptive_frames = min(NUM_FRAMES, max(2, native_frames - 2))
    print(f"  chunk_dur={chunk_dur:.2f}s  native_frames={native_frames}  num_frames={adaptive_frames}")

    # official PLM apply_chat_template
    # video_load_backend passed via processor_kwargs (required in newer transformers)
    # Retry with one fewer frame if the decoder reports out-of-bounds (rare on very short clips).
    for _nf in [adaptive_frames, adaptive_frames - 1, adaptive_frames - 2, 2]:
        if _nf < 2:
            continue
        try:
            inputs = processor.apply_chat_template(
                [conversation],
                processor_kwargs={"num_frames": _nf, "video_load_backend": "decord"},
                add_generation_prompt=True,
                tokenize=True,
                return_dict=True,
                return_tensors="pt",
            ).to(model.device)
            if _nf != adaptive_frames:
                print(f"    [retry] used num_frames={_nf} after IndexError at {adaptive_frames}")
            break
        except IndexError:
            continue
    else:
        raise RuntimeError(f"PLM: could not process {chunk_path} even at 2 frames")
